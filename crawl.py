...
def mysql_del():
    try:
        ...
        conn = pymysql.connect(host="localhost", user="root",
                               passwd=pw.passwd, db="jungang_meal", charset='utf8')
        # curs생성
        curs = conn.cursor()
    except:
...
def mysql_in(day, meal, when):
    ...
    # 데티어 생성
    sql = ("""INSERT INTO week_meal_"""+when +
           """(day, meal) VALUES ('""" + day + "', '" + meal + """')""")
    curs.execute(sql)
    conn.commit()
    conn.close()
...
def nies_parser(eduCode, schoolCode, when):
    ...
    url = 'http://stu.' + eduCode + '/sts_sci_md01_001.do?schulCode=' + schoolCode + \
        '&schulCrseScCode=4&schMmealScCode=' + \
        when + '&schYmd='+now.strftime('%Y.%m.%d')
    log.debug(url)
    ...
    while today < 7:
        ...
        menu = menu.replace('<td class="textC">', '').replace('</td>', '').replace('<br/>', '\n').replace(
            '(중앙)', '').replace('&amp;', '&').replace('(중앙', '').replace('(증앙)', '').replace('<td class="textC last">', '')
